...
# Get data (Substitute for appropriate benchmark file)
benchmarks <- read.delim(file = "workflow/stats/benchmarks.tsv",
                         sep = "\t", header = TRUE)
...
benchmarks <- benchmarks |>
  ...
  mutate(
    category = case_when(
      ...
      str_detect(rule, "star") ~ "Alignment",
      str_detect(rule, "expression") ~ "Expression"
    )
  )
